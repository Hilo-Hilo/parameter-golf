  ...
  if [ "$NO_VALIDATION" -eq 1 ]; then
    ...
    jq --arg branch "$BRANCH_NAME" \
       ...
       '.structured_output
        ...
        | .env_overrides = ((.env_overrides // {}) + (if (.env_overrides // {}).MAX_WALLCLOCK_SECONDS then {} else {MAX_WALLCLOCK_SECONDS: "5100"} end) + (if (.env_overrides // {}).TRAIN_BATCH_TOKENS then {} else {TRAIN_BATCH_TOKENS: "524288"} end) + {RUNPOD_OUTER_TIMEOUT_SECONDS: "10800"})
