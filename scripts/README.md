...
- `scripts/runpod_run_remote.sh`: Validates hardware (exact GPU count and model), applies an outer timeout, and starts the `run_experiment.sh` invocation inside `tmux`. It now preserves caller-provided `DATA_PATH` and `TOKENIZER_PATH` values instead of overwriting them, and otherwise defaults them to the shared `/workspace/parameter-golf/data` cache that bootstrap prepares.
