...
- `registry/observability/nodes/*.json` is the latest per-node controller status snapshot, including current phase, branch/job metadata, heartbeat age, and the latest mirrored log line.
- `registry/observability/jobs/<job_id>/` holds live mirrored remote artifacts such as `run.log`, heartbeat JSON, final-state JSON, and a per-job status snapshot.
...
### Controller Commands
- `scripts/start_swarm.sh [--no-validation] [--once]`: Bootstrap a controller run from a clean checkout. It ensures `registry/nodes.jsonl` exists, seeds `tree/<node_id>` from the requested base ref when missing, appends a pending node if there is no active pending/running entry yet, and then runs the supervisor once or in a loop. If the seed node is stuck in `running` but there is no live matching `branch_cycle.sh` process, the launcher recovers that stale entry back to `pending` automatically. Before doing any of that, it verifies the local Claude CLI is authenticated (either `claude auth login` for subscription auth or `ANTHROPIC_API_KEY` in the environment). Use `--reset-tree-ref` when you intentionally want to rebase the seed node back to the chosen base ref before restarting.
- `scripts/branch_cycle.sh [--no-validation] <node_id>`: Deterministic, phase-bounded 3-step Claude session (`plan`, `diagnose`, `reflect`) in an isolated Git worktree. The script acquires a per-node controller lock, reconciles stale pod leases, runs the plan phase, commits only scoped source/doc changes (excluding runtime state like `worktrees/`, `experiments/`, and `registry/`), pushes to GitHub, dispatches to RunPod, polls the remote tmux session with a controller TTL, collects artifacts, performs post-run pod cleanup, and only then runs the diagnosis/reflection phases. Claude phases run non-interactively (`stdin` from `/dev/null`), use the installed Claude CLI default model instead of pinning an older release, inline the JSON schema contents when calling `--json-schema`, and write persistent JSON/stderr artifacts under `registry/phase_logs/<node_id>/<scratch_ref>/` so a silent `plan` phase can still be inspected after failure. Before the `plan` phase starts, the controller now verifies the required upstream digest files exist locally and fails fast with a clear error if they do not. The controller also bundles compact plan-phase excerpts from the required large context files (`CLAUDE.md`, `PLAN.md`, `journal.md`, upstream digests, run tails, and spool file names) so Claude does not burn turns whole-reading files that exceed the `Read` tool token limit. Non-`success` Claude JSON subtypes (for example `error_max_turns`) and missing `structured_output` now count as immediate phase failures instead of surfacing later as a missing `proposed_slug`. During long remote waits, the controller mirrors the remote wrapper `run.log`, heartbeat JSON, and final-state JSON back into `registry/observability/jobs/<job_id>/`, updates the node/job status snapshots, and now prints branch-prefixed controller updates plus any newly mirrored remote `run.log` lines directly in the main terminal. If the Claude CLI itself returns an error payload, the controller now surfaces that message directly instead of only failing later on a missing `proposed_slug`. With `--no-validation`, the controller rewrites the dispatched job spec to the regular `pg-exp-*` 1xH100 lane and tags it as `non_record_h100x1` instead of routing to `pg-rec-*`.
...
- `scripts/watch_swarm.sh [--follow] [--node-id NODE_ID] [--job-id JOB_ID] [--log]`: Human-readable operator view over the controller event ledger and observability snapshots. Use this to see the current phase, branch, pod/SSH target, heartbeat age, last mirrored log line, recent controller events, and optionally the tail of the mirrored live `run.log`.
...
- If a controller phase fails before dispatch, `branch_cycle.sh` restores the parent node to `pending`, and `start_swarm.sh` will also recover a stale seed node whose `running` entry no longer has a live `branch_cycle.sh` behind it.
- If a controller phase fails after planning a child branch but before lease handoff, `branch_cycle.sh` now retracts the stale child node entry before requeueing the parent so duplicate fingerprints do not poison the next retry.
- When a Claude phase appears silent, inspect `registry/phase_logs/<node_id>/<scratch_ref>/`. Each phase now writes a JSON output file plus a matching `*.stderr.log` for operator debugging.
- Run `scripts/sync_upstream_context.sh` before unattended planning if the local `context/upstream/` digests are missing. `branch_cycle.sh` now refuses to start the `plan` phase without `issue_140.md`, `pr_digest.md`, and `frontier_digest.md`.
- If the `plan` phase repeatedly returns `error_max_turns`, inspect whether Claude is whole-reading large files. The controller now pre-bundles compact plan context specifically to avoid `Read` size-cap failures on files like `issue_140.md` and `registry/runs.jsonl`.
- The default operator view is now the main controller terminal itself: branch-tagged updates print inline, and once a pod run starts the controller streams newly mirrored remote `run.log` lines with the branch prefix.
- For a live terminal view, run `scripts/watch_swarm.sh --follow`. Add `--log` once a job has been dispatched to include the tail of the mirrored remote wrapper log.
