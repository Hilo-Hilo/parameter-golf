...
def eval_val_sliding_ttt(
    ...
    if args.ttt_muon:
        ttt_mom_bufs = {id(p): torch.zeros_like(p.data, dtype=torch.float32) for p in ttt_params}
        optimizer = None
    else:
                        ...
                        if args.ttt_muon:
                            with torch.no_grad():
                                for p in ttt_params:
                                    ...
                                    g = p.grad.float()
                                    if g.ndim >= 2:
                                        g = zeropower_via_newtonschulz5(g, steps=5).float()
                                        scale = max(g.size(-2), g.size(-1)) ** 0.5
                                    else:
                                        scale = 1.0
                                    buf = ttt_mom_bufs[id(p)]
                                    buf.mul_(args.ttt_momentum).add_(g, alpha=1.0)
                                    p.data.add_(buf * scale, alpha=-cos_lr)
                        else:
                            optimizer.step()
