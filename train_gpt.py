...
def eval_val_sliding_ttt(
    ...
    for name, p in base_model.named_parameters():
        ...
        if freeze:
            ...
            ttt_params.append(p)
            ttt_pg[_classify_param(name)].append(p)
    ...
    if args.ttt_optimizer == 'adamw':
        if args.ttt_perlayer_lr:
            _pg = []
            if ttt_pg['mlp']:
                _mlp_lr = args.ttt_lr * args.ttt_mlp_lr_scale
                _pg.append({'params': ttt_pg['mlp'], 'lr': _mlp_lr, 'initial_lr': _mlp_lr})
            if ttt_pg['attn']:
                _attn_lr = args.ttt_lr * args.ttt_attn_lr_scale
                _pg.append({'params': ttt_pg['attn'], 'lr': _attn_lr, 'initial_lr': _attn_lr})
            if ttt_pg['other']:
                _pg.append({'params': ttt_pg['other'], 'lr': args.ttt_lr, 'initial_lr': args.ttt_lr})
...
def main() -> None:
    ...
    _lzma_preset = getattr(args, 'lzma_preset', 6)
    _lzma_eff = _lzma_preset | (lzma.PRESET_EXTREME if getattr(args, 'lzma_extreme', False) else 0)
    quant_blob = lzma.compress(quant_raw, preset=_lzma_eff)
    if master_process:
