    ...
    for name, p in base_model.named_parameters():
        ...
        if freeze:
            ...
            if args.ttt_perlayer:
                if "mlp_" in name or ".mlp." in name:
                    mult = 3.0
                elif ".attn." in name or "qo_bank" in name or "kv_bank" in name:
                    mult = 0.5
                else:
                    mult = 1.0
                ttt_param_groups.append({"params": [p], "lr": args.ttt_lr * mult, "base_lr": args.ttt_lr * mult})

    log0(f"ttt_sliding:params unfrozen={sum(p.numel() for p in ttt_params)} "
         f"frozen={sum(p.numel() for p in base_model.parameters() if not p.requires_grad)}")

    if args.ttt_optimizer == "adamw":
        if args.ttt_perlayer and ttt_param_groups:
            optimizer = torch.optim.AdamW(ttt_param_groups, lr=args.ttt_lr, weight_decay=0.0, betas=(0.9, 0.999))
        else:
