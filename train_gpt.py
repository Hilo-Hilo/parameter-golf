...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer == "adamw":
        if args.ttt_perlayer_lr:
            ...
            for name, p in zip(ttt_names, ttt_params):
            ...
            pgs = []
            if mlp_ps:
                pgs.append({"params": mlp_ps, "lr": base * args.ttt_perlayer_mlp_scale,
                            "base_lr": base * args.ttt_perlayer_mlp_scale})
            if attn_ps:
                pgs.append({"params": attn_ps, "lr": base * args.ttt_perlayer_attn_scale,
                            "base_lr": base * args.ttt_perlayer_attn_scale})
            if other_ps:
                pgs.append({"params": other_ps, "lr": base, "base_lr": base})
            optimizer = torch.optim.AdamW(pgs, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.0)
            log0(f"ttt_sliding:optimizer=adamw perlayer=True mlp_scale={args.ttt_perlayer_mlp_scale} "
                 f"attn_scale={args.ttt_perlayer_attn_scale} mlp_params={sum(p.numel() for p in mlp_ps)} "
                 f"attn_params={sum(p.numel() for p in attn_ps)}")
        else:
            optimizer = torch.optim.AdamW(ttt_params, lr=args.ttt_lr, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.0)
            log0(f"ttt_sliding:optimizer=adamw perlayer=False lr={args.ttt_lr}")
    else:
