...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer.lower() == "adamw":
        if args.ttt_perlayer_lr:
            ...
            for n, p in zip(ttt_param_names, ttt_params):
                if "mlp" in n:
                    mlp_ps.append(p)
                elif "qo" in n or "kv" in n:
                    attn_ps.append(p)
                else:
                    other_ps.append(p)
            param_groups = []
            if mlp_ps:
                param_groups.append({"params": mlp_ps, "lr": mlp_lr, "initial_lr": mlp_lr})
            if attn_ps:
                param_groups.append({"params": attn_ps, "lr": attn_lr, "initial_lr": attn_lr})
            if other_ps:
                param_groups.append({"params": other_ps, "lr": args.ttt_lr, "initial_lr": args.ttt_lr})
...
def main() -> None:
    ...
    _lzma_preset = args.lzma_preset | (lzma.PRESET_EXTREME if args.lzma_extreme else 0)
