...
def eval_val_sliding_ttt(
    ...
    for name, p in base_model.named_parameters():
        ...
        if freeze:
            ...
            if 'mlp' in name:
                mlp_params.append(p)
            elif any(k in name for k in ('c_q', 'c_k', 'c_v', 'attn')):
    ...
    if args.ttt_optimizer == "adamw":
        if args.ttt_perlayer_lr:
            param_groups = [
                {'params': mlp_params, 'lr': args.ttt_lr * 3.0, 'base_lr': args.ttt_lr * 3.0},
                {'params': attn_params, 'lr': args.ttt_lr * 0.5, 'base_lr': args.ttt_lr * 0.5},
                {'params': other_params, 'lr': args.ttt_lr, 'base_lr': args.ttt_lr},
            ]
            optimizer = torch.optim.AdamW([pg for pg in param_groups if pg['params']], weight_decay=0.0)
        else:
