...
class Hyperparameters:
    ...
    int5_cats = set(s.strip() for s in os.environ.get("INT5_CATS", "").split(",") if s.strip())
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], int5_cats: set[str] | None = None):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            use_int5 = bool(int5_cats and cat in int5_cats)
            clip = 15 if use_int5 else 31
...
def main() -> None:
    ...
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, args.int5_cats if args.int5_cats else None)
