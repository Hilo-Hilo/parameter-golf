...
class Hyperparameters:
    ...
    ttt_use_adamw = bool(int(os.environ.get("TTT_USE_ADAMW", "1")))
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_use_adamw:
        _attn = {'qo_bank', 'kv_bank'}
        _mlp = {'mlp_up_bank', 'mlp_down_bank'}
        attn_pg = [p for n, p in zip(ttt_param_names, ttt_params) if n in _attn]
        mlp_pg = [p for n, p in zip(ttt_param_names, ttt_params) if n in _mlp]
        other_pg = [p for n, p in zip(ttt_param_names, ttt_params)
                    if n not in _attn and n not in _mlp]
        groups: list[dict] = []
        if attn_pg:
            lr_a = args.ttt_lr * args.ttt_attn_lr_mult
            groups.append({'params': attn_pg, 'lr': lr_a, 'initial_lr': lr_a})
        if mlp_pg:
            lr_m = args.ttt_lr * args.ttt_mlp_lr_mult
            groups.append({'params': mlp_pg, 'lr': lr_m, 'initial_lr': lr_m})
        if other_pg:
            groups.append({'params': other_pg, 'lr': args.ttt_lr, 'initial_lr': args.ttt_lr})
