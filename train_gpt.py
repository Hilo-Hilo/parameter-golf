...
class TrigramHashEmbedding(nn.Module):
    ...
    def trigram_hash(self, tokens: Tensor) -> Tensor:
        ...
        out[..., 2:] = torch.bitwise_xor(
            36313 * t[..., 2:],
            torch.bitwise_xor(27191 * t[..., 1:-1], 11173 * t[..., :-2])
        ) % mod
        return out.long()
    def forward(self, token_ids: Tensor) -> Tensor:
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer == "adamw":
        if args.ttt_perlayer_lr:
            pgroups = [{"params": [p], "lr": args.ttt_lr * _ttt_lr_mult(n), "base_lr": args.ttt_lr * _ttt_lr_mult(n)}
                       for n, p in base_model.named_parameters() if p.requires_grad]
            optimizer = torch.optim.AdamW(pgroups, betas=(0.9, 0.95), weight_decay=0.0, eps=1e-8)
        else:
