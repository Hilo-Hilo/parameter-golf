...
class DistributedTokenLoader:
    def __init__(self, pattern: str, rank: int, world_size: int, device: torch.device):
        ...
        # Coprime-stride: assign each rank a different starting shard so GPUs see
        # diverse training data rather than adjacent sequential blocks.
        coprime_stride = os.environ.get("COPRIME_STRIDE", "0") == "1"
        if coprime_stride and len(self.stream.files) > 1:
            n = len(self.stream.files)
            stride = world_size
            while math.gcd(stride, n) != 1:
                stride += 1
            start_idx = (rank * stride) % n
            self.stream.files = [self.stream.files[(start_idx + i) % n] for i in range(n)]
            self.stream.file_idx = 0
            self.stream.tokens = load_data_shard(self.stream.files[0])
            self.stream.pos = 0
            self._independent_stream = True
        else:
            self._independent_stream = False
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        local_tokens = global_tokens // (self.world_size * grad_accum_steps)
        per_rank_span = local_tokens + 1
        if self._independent_stream:
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_perlayer_lr and args.ttt_optimizer == "adamw":
        # Per-bank param groups: MLP 2x lr, attention 0.5x, others 1x
