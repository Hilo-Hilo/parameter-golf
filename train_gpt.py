...
class GPT(nn.Module):
    def __init__(
        ...
        self.skip_weights = nn.Parameter(torch.zeros(self.num_skip_weights, dtype=torch.float32))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            if skips:
                x = x + torch.sigmoid(self.skip_weights[i].to(dtype=x.dtype)) * skips.pop()
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer == "adamw":
        if args.ttt_perlayer:
            _mlp_down = [p for n, p in ttt_named_params if "mlp_down_bank" in n]
            _attn = [p for n, p in ttt_named_params if "qo_bank" in n or "kv_bank" in n]
            _other = [p for n, p in ttt_named_params if "mlp_down_bank" not in n and "qo_bank" not in n and "kv_bank" not in n]
            _pg = []
            if _mlp_down:
                _pg.append({'params': _mlp_down, 'lr': args.ttt_lr * 3.0, 'initial_lr': args.ttt_lr * 3.0})
            if _attn:
                _pg.append({'params': _attn, 'lr': args.ttt_lr * 0.5, 'initial_lr': args.ttt_lr * 0.5})
            if _other:
                _pg.append({'params': _other, 'lr': args.ttt_lr, 'initial_lr': args.ttt_lr})
            optimizer = torch.optim.AdamW(_pg, betas=(0.9, 0.95), weight_decay=0.0)
        else:
