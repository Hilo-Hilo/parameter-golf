...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer == "adamw":
        if args.ttt_perlayer:
            down_params = [p for n, p in base_model.named_parameters() if "mlp_down_bank" in n and p.requires_grad]
            attn_params = [p for n, p in base_model.named_parameters() if ("qo_bank" in n or "kv_bank" in n) and p.requires_grad]
            attn_up_ids = {id(q) for q in down_params + attn_params}
            other_params = [p for p in ttt_params if id(p) not in attn_up_ids]
            optimizer = torch.optim.AdamW([
                {"params": down_params, "lr": args.ttt_lr * 3.0, "base_lr_scale": 3.0},
                {"params": attn_params, "lr": args.ttt_lr * 0.5, "base_lr_scale": 0.5},
                {"params": other_params, "lr": args.ttt_lr, "base_lr_scale": 1.0},
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                ...
                for pg in optimizer.param_groups:
                    pg['lr'] = args.ttt_lr * pg.get('base_lr_scale', 1.0) * cos_scale
