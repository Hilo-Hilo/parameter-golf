...
class Hyperparameters:
    ...
    ttt_perlayer_mlp_scale = float(os.environ.get("TTT_PERLAYER_MLP_SCALE", "2.0"))
    ttt_perlayer_attn_scale = float(os.environ.get("TTT_PERLAYER_ATTN_SCALE", "0.5"))
...
def eval_val_sliding_ttt(
    ...
    log0(f"ttt_sliding:optimizer={args.ttt_optimizer} perlayer_lr={args.ttt_perlayer_lr}")

    if args.ttt_optimizer == "adamw":
        if args.ttt_perlayer_lr:
            _mlp_keys = ("mlp_up_bank", "mlp_down_bank")
            _attn_keys = ("qo_bank", "kv_bank")
            mlp_ps = [p for n, p in base_model.named_parameters() if p.requires_grad and any(k in n for k in _mlp_keys)]
            attn_ps = [p for n, p in base_model.named_parameters() if p.requires_grad and any(k in n for k in _attn_keys)]
            other_ps = [p for n, p in base_model.named_parameters() if p.requires_grad and not any(k in n for k in _mlp_keys + _attn_keys)]
            ttt_param_groups = [
                {"params": mlp_ps, "lr": args.ttt_lr * args.ttt_perlayer_mlp_scale, "base_lr": args.ttt_lr * args.ttt_perlayer_mlp_scale},
                {"params": attn_ps, "lr": args.ttt_lr * args.ttt_perlayer_attn_scale, "base_lr": args.ttt_lr * args.ttt_perlayer_attn_scale},
                {"params": other_ps, "lr": args.ttt_lr, "base_lr": args.ttt_lr},
            ]
        else:
            ttt_param_groups = [{"params": ttt_params, "lr": args.ttt_lr, "base_lr": args.ttt_lr}]
        optimizer = torch.optim.AdamW(ttt_param_groups, betas=(args.beta1, args.beta2), weight_decay=0.01)
    else:
...
def main() -> None:
    ...
    if master_process:
        ...
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model int6+lzma{args.lzma_preset}: {quant_file_bytes} bytes")
        log0(f"Total submission size int6+lzma{args.lzma_preset}: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
