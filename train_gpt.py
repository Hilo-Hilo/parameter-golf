...
class Hyperparameters:
    ...
    ttt_entropy_weight = float(os.environ.get("TTT_ENTROPY_WEIGHT", "0.0"))
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        if args.ttt_entropy_weight > 0.0:
                            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                            ...
                            with torch.no_grad():
                                w = torch.softmax(
                                    per_tok_nll.detach() * args.ttt_entropy_weight, dim=0
                                ).mul_(per_tok_nll.numel())
                            loss = (per_tok_nll * w).mean()
                        else:
                            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                                loss = base_model(x, y)
                        loss.backward()
