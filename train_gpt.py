...
# --- MuonTTT optimizer (NS5 for test-time training) ---

class MuonTTT(torch.optim.Optimizer):
    """Muon-style optimizer for TTT: NS5 orthogonalization for 2D params,
    momentum SGD for 1D params. Non-distributed -- caller all_reduces grads
    before calling step()."""
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                buf = state['buf']
                buf.mul_(mom).add_(g)
                update = g.add(buf, alpha=mom) if nesterov else buf.clone()
                if update.ndim == 2:
                    update = zeropower_via_newtonschulz5(update, steps=steps).to(p.dtype)
...
def eval_val_sliding_ttt(
    ...
    if getattr(args, 'ttt_muon', False):
