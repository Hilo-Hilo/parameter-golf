...
def eval_val_sliding_ttt(
    ...
    def _is_mlp(n): return ".mlp." in n or "mlp_up_bank" in n or "mlp_down_bank" in n
    def _is_attn(n): return ".attn." in n or "qo_bank" in n or "kv_bank" in n
    if args.ttt_optimizer == "adamw":
        if args.ttt_perlayer_lr:
            mlp_ps = [p for n, p in ttt_named_params if _is_mlp(n)]
            attn_ps = [p for n, p in ttt_named_params if not _is_mlp(n) and _is_attn(n)]
            other_ps = [p for n, p in ttt_named_params if not _is_mlp(n) and not _is_attn(n)]
            optimizer = torch.optim.AdamW([
                {'params': mlp_ps, 'lr': args.ttt_lr * args.ttt_mlp_lr_scale, 'initial_lr': args.ttt_lr * args.ttt_mlp_lr_scale},
                {'params': attn_ps, 'lr': args.ttt_lr * args.ttt_attn_lr_scale, 'initial_lr': args.ttt_lr * args.ttt_attn_lr_scale},
                {'params': other_ps, 'lr': args.ttt_lr, 'initial_lr': args.ttt_lr},
            ], weight_decay=0.0)
        else:
