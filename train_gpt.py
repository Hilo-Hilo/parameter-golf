...
class Hyperparameters:
    ...
    ttt_adamw_beta2 = float(os.environ.get("TTT_ADAMW_BETA2", 0.95))
    ttt_adamw_eps = float(os.environ.get("TTT_ADAMW_EPS", 1e-8))
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer == "adamw":
        # Per-module AdamW: MLP params get ttt_mlp_lr_mult * ttt_lr, others get ttt_lr
        ttt_named = [(n, p) for n, p in base_model.named_parameters() if p.requires_grad]
        mlp_params = [p for n, p in ttt_named if ".mlp." in n]
        other_params = [p for n, p in ttt_named if ".mlp." not in n]
        param_groups = [
            {"params": mlp_params, "lr": args.ttt_lr * args.ttt_mlp_lr_mult, "base_lr": args.ttt_lr * args.ttt_mlp_lr_mult},
            {"params": other_params, "lr": args.ttt_lr, "base_lr": args.ttt_lr},
        ]
        optimizer = torch.optim.AdamW(
            param_groups,
            betas=(args.ttt_adamw_beta1, args.ttt_adamw_beta2),
            eps=args.ttt_adamw_eps,
            weight_decay=0.0,
        )
        log0(f"ttt_sliding:optimizer=adamw mlp_params={sum(p.numel() for p in mlp_params)} "
             f"other_params={sum(p.numel() for p in other_params)} "
             f"mlp_lr={args.ttt_lr * args.ttt_mlp_lr_mult:.5f} other_lr={args.ttt_lr:.5f}")
    else:
