...
def eval_val_sliding_ttt(
    ...
    if args.ttt_use_adamw:
        if args.ttt_perlayer_lr:
            mlp_ps = [p for n, p in ttt_named_params if _classify_param(n) == "mlp"]
            attn_ps = [p for n, p in ttt_named_params if _classify_param(n) == "attn"]
            other_ps = [p for n, p in ttt_named_params if _classify_param(n) not in ("mlp", "attn")]
            pgs = [
                {"params": mlp_ps, "lr": args.ttt_lr * args.ttt_mlp_lr_mult, "base_lr_mult": args.ttt_mlp_lr_mult},
                {"params": attn_ps, "lr": args.ttt_lr * args.ttt_attn_lr_mult, "base_lr_mult": args.ttt_attn_lr_mult},
                {"params": other_ps, "lr": args.ttt_lr, "base_lr_mult": 1.0},
            ]
            pgs = [pg for pg in pgs if len(pg["params"]) > 0]
            optimizer = torch.optim.AdamW(pgs, betas=(0.9, 0.999), weight_decay=0.0, eps=1e-8)
        else:
            optimizer = torch.optim.AdamW(ttt_params, lr=args.ttt_lr, betas=(0.9, 0.999), weight_decay=0.0, eps=1e-8)
    else:
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                if not args.ttt_cosepoch:
                    chunk_lr_scale = 0.5 * (1.0 + math.cos(math.pi * ci / max(num_chunks - 1, 1)))
                    for pg in optimizer.param_groups:
                        pg['lr'] = args.ttt_lr * pg.get("base_lr_mult", 1.0) * chunk_lr_scale
                my_seq_s = (chunk_seqs * rank) // world_size
                my_seq_e = (chunk_seqs * (rank + 1)) // world_size
                my_chunk_seqs = my_seq_e - my_seq_s
                for _ep in range(args.ttt_epochs):
                    if args.ttt_cosepoch:
                        epoch_lr_scale = 0.5 * (1.0 + math.cos(math.pi * _ep / max(args.ttt_epochs, 1)))
                        for pg in optimizer.param_groups:
                            pg['lr'] = args.ttt_lr * pg.get("base_lr_mult", 1.0) * epoch_lr_scale
