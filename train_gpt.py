...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer == "adamw":
        if args.ttt_perlayer_lr:
            pg_mlp_up, pg_mlp_down, pg_qo, pg_kv, pg_other = [], [], [], [], []
            for name, p in base_model.named_parameters():
            ...
            optimizer = torch.optim.AdamW(
                [
                    {"params": pg_mlp_up, "lr": args.ttt_lr * 2.0, "initial_lr": args.ttt_lr * 2.0},
                    {"params": pg_mlp_down, "lr": args.ttt_lr * 2.0, "initial_lr": args.ttt_lr * 2.0},
                    {"params": pg_qo, "lr": args.ttt_lr * 0.5, "initial_lr": args.ttt_lr * 0.5},
                    {"params": pg_kv, "lr": args.ttt_lr * 0.5, "initial_lr": args.ttt_lr * 0.5},
                    {"params": pg_other, "lr": args.ttt_lr, "initial_lr": args.ttt_lr},
                ],
                betas=(0.9, 0.95), eps=1e-8, weight_decay=0.0,
            )
        else:
            optimizer = torch.optim.AdamW(
                ttt_params, lr=args.ttt_lr, betas=(0.9, 0.95), eps=1e-8, weight_decay=0.0,
            )
    else:
