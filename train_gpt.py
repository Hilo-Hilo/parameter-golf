...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer == "adamw":
        mlp_ttt, attn_ttt, other_ttt = [], [], []
        for name, p in base_model.named_parameters():
            if not p.requires_grad:
                continue
            if any(k in name for k in ("mlp_up_bank", "mlp_down_bank", ".mlp.")):
                mlp_ttt.append(p)
            elif any(k in name for k in ("qo_bank", "kv_bank", ".attn.")):
                attn_ttt.append(p)
            else:
                other_ttt.append(p)
        ttt_param_groups = [
            {"params": other_ttt, "lr": args.ttt_lr, "_base_lr_ratio": 1.0},
            {"params": mlp_ttt, "lr": args.ttt_lr * args.ttt_mlp_lr_mult, "_base_lr_ratio": args.ttt_mlp_lr_mult},
            {"params": attn_ttt, "lr": args.ttt_lr * args.ttt_attn_lr_mult, "_base_lr_ratio": args.ttt_attn_lr_mult},
        ]
        optimizer = torch.optim.AdamW(ttt_param_groups, betas=(0.9, 0.999), weight_decay=0.0)
    else:
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                ...
                for pg in optimizer.param_groups:
                    base_ratio = pg.get('_base_lr_ratio', 1.0)
                    pg['lr'] = args.ttt_lr * cos_factor * base_ratio
