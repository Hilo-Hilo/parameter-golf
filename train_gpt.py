...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            clip_range = CastedLinear._qat_clip_range
            with torch.no_grad():
                w32 = self.weight.float()
                row_max = w32.abs().amax(dim=1)
                scale = (row_max / clip_range).clamp_min(1.0 / clip_range)
                w_q = (torch.clamp(torch.round(w32 / scale[:, None]), -clip_range, clip_range) * scale[:, None]).to(x.dtype)
            w = w + (w_q - w).detach()
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer == "adamw":
        if args.ttt_perlayer_lr:
            ttt_param_names = {id(p): n for n, p in base_model.named_parameters()}
            param_groups: list[dict] = []
            for p in ttt_params:
                name = ttt_param_names.get(id(p), "")
                if ".mlp." in name or "mlp_up_bank" in name or "mlp_down_bank" in name:
                    ...
                    lr_mult = 1.0
                param_groups.append({"params": [p], "lr": args.ttt_lr * lr_mult, "lr_mult": lr_mult})
