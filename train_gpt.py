...
# Module-level leaky slope constant (read once at import time)
_LEAKY_SLOPE: float = float(os.environ.get("LEAKY_SLOPE", "0.5"))
...
class TrigramHashEmbedding(nn.Module):
    """Hash-based trigram (3-gram) context embedding: combines 3 consecutive tokens."""
    ...
    def trigram_hash(self, tokens: Tensor) -> Tensor:
        ...
        out[..., 2:] = (36313 * t[..., 2:] ^ 27191 * t[..., 1:-1] ^ 54331 * t[..., :-2]) % mod
        return out.long()
    def forward(self, token_ids: Tensor) -> Tensor:
...
def eval_val_sliding_ttt(
    ...
    mlp_down_ttt, attn_ttt, other_ttt = [], [], []
    for name, p in base_model.named_parameters():
        ...
        if freeze:
            ...
            if "mlp_down_bank" in name:
                mlp_down_ttt.append(p)
            elif "qo_bank" in name or "kv_bank" in name:
                attn_ttt.append(p)
            else:
                other_ttt.append(p)
    ttt_params = mlp_down_ttt + attn_ttt + other_ttt

    log0(f"ttt_sliding:params unfrozen={sum(p.numel() for p in ttt_params)} "
         f"frozen={sum(p.numel() for p in base_model.parameters() if not p.requires_grad)}")

    use_adamw = getattr(args, "ttt_optimizer", "sgd") == "adamw"
    use_perlayer = getattr(args, "ttt_perlayer", False)
    if use_adamw:
        base_lr = args.ttt_lr
        if use_perlayer:
            param_groups = [
                {"params": mlp_down_ttt, "lr": base_lr * 3.0, "initial_lr": base_lr * 3.0},
                {"params": attn_ttt, "lr": base_lr * 0.5, "initial_lr": base_lr * 0.5},
                {"params": other_ttt, "lr": base_lr, "initial_lr": base_lr},
            ]
        else:
            param_groups = [
                {"params": ttt_params, "lr": base_lr, "initial_lr": base_lr},
            ]
