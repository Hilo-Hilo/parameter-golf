...
class Hyperparameters:
    ...
    ttt_qo_only = bool(int(os.environ.get("TTT_QO_ONLY", "0")))
    ttt_freeze_mlp = bool(int(os.environ.get("TTT_FREEZE_MLP", "0")))
    ...
    for name, p in base_model.named_parameters():
        ...
        for bi in frozen_block_ids:
            if f"blocks.{bi}." in name:
                freeze = True
                break
        if args.ttt_qo_only and name in ("kv_bank", "mlp_up_bank", "mlp_down_bank"):
            freeze = True
        if args.ttt_freeze_mlp and name in ("mlp_up_bank", "mlp_down_bank"):
            freeze = True
        if freeze:
                        ...
                        if optimizer is not None:
                            ...
                            with torch.no_grad():
                                for p in ttt_params:
                                    ...
                                    g_nes = g.add(buf, alpha=args.ttt_momentum)
                                    if g_nes.ndim >= 2 and min(g_nes.shape[-2:]) > 1:
                                        scale = max(1.0, g_nes.shape[-2] / g_nes.shape[-1]) ** 0.5
                                        g_nes = zeropower_via_newtonschulz5(g_nes, steps=5)
                                        p.data.add_(g_nes.to(p.dtype), alpha=-cos_lr * scale)
                                    else:
                                        p.data.add_(g_nes.float().to(p.dtype), alpha=-cos_lr)
