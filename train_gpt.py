...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer == "adamw":
        if args.ttt_perlayer_lr:
            ...
            pg_list = []
            block_param_ids: set = set()
            for bi, block in enumerate(base_model.blocks):
                bparams = [p for p in block.parameters() if p.requires_grad]
                if bparams:
                    lr_scale = (bi + 1) / num_blocks
                    pg_list.append({"params": bparams, "lr": args.ttt_lr * lr_scale, "_base_lr": args.ttt_lr * lr_scale})
                    block_param_ids.update(id(p) for p in bparams)
            other_params = [p for p in ttt_params if id(p) not in block_param_ids]
            if other_params:
                pg_list.append({"params": other_params, "lr": args.ttt_lr, "_base_lr": args.ttt_lr})
            optimizer = torch.optim.AdamW(pg_list, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.0)
        else:
