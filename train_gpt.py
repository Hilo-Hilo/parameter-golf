...
class Hyperparameters:
    ...
    slot_ttt_epochs = int(os.environ.get("SLOT_TTT_EPOCHS", "20"))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.slot_vector = nn.Parameter(torch.zeros(model_dim)) if slot_enabled else None
        self._init_weights()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
        if self.slot_vector is not None:
            x = x + self.slot_vector.to(dtype=x.dtype)[None, None, :]
...
def eval_val_sliding_ttt(
    ...
    # Freeze first N blocks (or use SLOT-only TTT)
    if args.slot_enabled and hasattr(base_model, 'slot_vector') and base_model.slot_vector is not None:
        for p in base_model.parameters():
            p.requires_grad_(False)
        base_model.slot_vector.requires_grad_(True)
        ttt_params = [base_model.slot_vector]
    else:
    ...
    for ci in range(num_chunks):
        ...
        effective_ttt_epochs = args.slot_ttt_epochs if (args.slot_enabled and hasattr(base_model, 'slot_vector') and base_model.slot_vector is not None) else args.ttt_epochs
        if not is_last_chunk and effective_ttt_epochs > 0:
            base_model.train()
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(effective_ttt_epochs):
