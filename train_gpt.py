...
class Hyperparameters:
    ...
    ttt_gate_lr_scale = float(os.environ.get("TTT_GATE_LR_SCALE", "1.0"))
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_gate_lr_scale != 1.0:
        gate_ids = {id(p) for name, p in base_model.named_parameters() if "attn_gate" in name and p.requires_grad}
        gate_p = [p for p in ttt_params if id(p) in gate_ids]
        other_p = [p for p in ttt_params if id(p) not in gate_ids]
        optimizer = torch.optim.AdamW(
            [{"params": other_p, "lr": args.ttt_lr},
             {"params": gate_p, "lr": args.ttt_lr * args.ttt_gate_lr_scale}],
