...
class Hyperparameters:
    ...
    ttt_mlp_down_lr_mult = float(os.environ.get("TTT_MLP_DOWN_LR_MULT", "3.0"))
    ttt_mlp_up_lr_mult = float(os.environ.get("TTT_MLP_UP_LR_MULT", "0.5"))
    ...
    ttt_epoch_lr_decay = float(os.environ.get("TTT_EPOCH_LR_DECAY", "1.0"))
        ...
        if args.ttt_per_layer_lr:
            ...
            for name, p in base_model.named_parameters():
                ...
                if "mlp.proj" in name:
                    ...
                    other_p.append(p)
            pg_list = [
                {"params": down_p, "lr": args.ttt_lr * args.ttt_mlp_down_lr_mult, "base_lr": args.ttt_lr * args.ttt_mlp_down_lr_mult},
                {"params": up_p, "lr": args.ttt_lr * args.ttt_mlp_up_lr_mult, "base_lr": args.ttt_lr * args.ttt_mlp_up_lr_mult},
                {"params": attn_p, "lr": args.ttt_lr * args.ttt_attn_lr_mult, "base_lr": args.ttt_lr * args.ttt_attn_lr_mult},
                {"params": other_p, "lr": args.ttt_lr, "base_lr": args.ttt_lr},
            ]
            optimizer = torch.optim.AdamW(pg_list, betas=(args.ttt_adamw_beta1, args.ttt_adamw_beta2), weight_decay=0.0)
        else:
    ...
    for ci in range(num_chunks):
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                ...
                if not args.ttt_use_adamw:
                    cos_lr = args.ttt_lr * chunk_cos
                ...
                for _ep in range(args.ttt_epochs):
                    if args.ttt_use_adamw:
                        ep_decay = (args.ttt_epoch_lr_decay ** _ep) * 0.5 * (1.0 + math.cos(math.pi * _ep / max(args.ttt_epochs, 1)))
                        for pg in optimizer.param_groups:
                            pg['lr'] = pg['base_lr'] * chunk_cos * ep_decay
