...
def eval_val_sliding_ttt(
    ...
    if args.ttt_perlayer_lr:
        mlp_ids = set(id(p) for name, p in base_model.named_parameters()
                      if p.requires_grad and ('mlp_up_bank' in name or 'mlp_down_bank' in name))
        attn_ids = set(id(p) for name, p in base_model.named_parameters()
                       if p.requires_grad and ('qo_bank' in name or 'kv_bank' in name or '.attn.' in name))
        pg_mlp = [p for p in ttt_params if id(p) in mlp_ids]
        pg_attn = [p for p in ttt_params if id(p) in attn_ids]
        pg_other = [p for p in ttt_params if id(p) not in mlp_ids and id(p) not in attn_ids]
        ttt_param_groups = []
        if pg_mlp:
            ttt_param_groups.append({'params': pg_mlp, 'lr': args.ttt_lr * args.ttt_mlp_lr_mult,
                                     'initial_lr': args.ttt_lr * args.ttt_mlp_lr_mult})
        if pg_attn:
            ttt_param_groups.append({'params': pg_attn, 'lr': args.ttt_lr * args.ttt_attn_lr_mult,
                                     'initial_lr': args.ttt_lr * args.ttt_attn_lr_mult})
        if pg_other:
            ttt_param_groups.append({'params': pg_other, 'lr': args.ttt_lr,
                                     'initial_lr': args.ttt_lr})
    else:
        ttt_param_groups = [{'params': ttt_params, 'lr': args.ttt_lr, 'initial_lr': args.ttt_lr}]

    if args.ttt_optimizer == "adamw":
        optimizer = torch.optim.AdamW(ttt_param_groups, lr=args.ttt_lr, weight_decay=0.0)
    else:
