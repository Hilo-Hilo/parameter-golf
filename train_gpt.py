        ...
        if args.ttt_perlayer_lr:
            ...
            for bi, block in enumerate(base_model.blocks):
                if bi in frozen_block_ids:
                    continue
                lr_scale = (bi + 1) / len(base_model.blocks)
                block_params = [p for p in block.parameters() if p.requires_grad]
                if block_params:
                    param_groups.append({"params": block_params, "lr": args.ttt_lr * lr_scale, "_base_lr": args.ttt_lr * lr_scale})
            ...
            if other_params:
                param_groups.append({"params": other_params, "lr": args.ttt_lr, "_base_lr": args.ttt_lr})
