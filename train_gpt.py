...
def eval_val_sliding_ttt(
    ...
    _qo_hook = None
    if args.ttt_qonly:
        # Query-only TTT: only update Q projection (qo_bank[0:N]) via gradient masking
        for p in base_model.parameters():
            p.requires_grad_(False)
        base_model.qo_bank.requires_grad_(True)
        ttt_params = [base_model.qo_bank]
        _n_layers = args.num_layers
        def _mask_o_grads(grad):
            g = grad.clone()
            g[_n_layers:] = 0.0
            return g
        _qo_hook = base_model.qo_bank.register_hook(_mask_o_grads)
    else:
