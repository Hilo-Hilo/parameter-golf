...
def eval_val_sliding_ttt(
    ...
    # AdamW TTT with per-layer LR: MLP params 2x, attention params 0.5x, other 1x
    ttt_id_set = set(id(p) for p in ttt_params)
    mlp_ps = [p for n, p in base_model.named_parameters() if id(p) in ttt_id_set and '.mlp.' in n]
    attn_ps = [p for n, p in base_model.named_parameters() if id(p) in ttt_id_set and ('.attn.' in n or 'qo_bank' in n or 'kv_bank' in n)]
    other_ps = [p for p in ttt_params if id(p) not in set(id(x) for x in mlp_ps + attn_ps)]
    optimizer = torch.optim.AdamW([
        {'params': mlp_ps, 'lr': args.ttt_lr * 2.0, '_base_lr': args.ttt_lr * 2.0},
        {'params': attn_ps, 'lr': args.ttt_lr * 0.5, '_base_lr': args.ttt_lr * 0.5},
        {'params': other_ps, 'lr': args.ttt_lr, '_base_lr': args.ttt_lr},
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                ...
                for pg in optimizer.param_groups:
                    pg['lr'] = pg.get('_base_lr', args.ttt_lr) * cos_frac
