                        ...
                        if args.ttt_muon:
                            with torch.no_grad():
                                for p in ttt_params:
                                    ...
                                    g = p.grad.bfloat16()
                                    pid = id(p)
                                    if pid not in muon_bufs:
                                        muon_bufs[pid] = torch.zeros_like(g)
                                    buf = muon_bufs[pid]
                                    buf.mul_(args.ttt_momentum).add_(g)
                                    update_g = g.add(buf, alpha=args.ttt_momentum)
                                    if g.ndim >= 2:
                                        scale = max(1.0, g.shape[-2] / g.shape[-1]) ** 0.5
                                        update = zeropower_via_newtonschulz5(update_g, steps=args.muon_backend_steps)
                                        p.data.add_(update.to(p.dtype), alpha=-cos_lr * scale)
                                    else:
                                        g_norm = update_g.norm().item()
                                        if g_norm > args.ttt_grad_clip:
                                            update_g = update_g * (args.ttt_grad_clip / (g_norm + 1e-8))
                                        p.data.add_(update_g.to(p.dtype), alpha=-cos_lr)
                        else:
