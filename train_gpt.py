...
def eval_val_sliding_ttt(
    ...
    if args.ttt_muon:
        ttt_muon_buf = {id(p): torch.zeros_like(p.data, dtype=torch.bfloat16) for p in ttt_params}
        optimizer = None
    else:
        ttt_muon_buf = {}
                            ...
                            with torch.no_grad():
                                for p in ttt_params:
                                    if p.grad is None:
                                        continue
                                    g = p.grad.bfloat16()
                                    buf = ttt_muon_buf[id(p)]
                                    buf.mul_(args.ttt_momentum).add_(g)
                                    nesterov_g = g.add(buf, alpha=args.ttt_momentum)
                                    if nesterov_g.ndim == 3:
                                        update = torch.stack([
                                            zeropower_via_newtonschulz5(nesterov_g[i], steps=args.muon_backend_steps)
                                            for i in range(nesterov_g.shape[0])
                                        ])
                                    elif nesterov_g.ndim == 2:
                                        update = zeropower_via_newtonschulz5(nesterov_g, steps=args.muon_backend_steps)
                                    else:
                                        update = nesterov_g
                                    scale = max(1.0, (update.shape[-2] / update.shape[-1]) ** 0.5) if update.ndim >= 2 else 1.0
                                    p.data.add_(update.to(dtype=p.dtype), alpha=-cos_lr * scale)
                        else:
