...
def eval_val_sliding_ttt(
    ...
    # Freeze first N blocks and build param groups for TTT optimizer
    ...
    _use_adamw = getattr(args, 'ttt_optimizer', 'sgd') == 'adamw'
    _use_perlayer = _use_adamw and getattr(args, 'ttt_perlayer_lr', False)
    if _use_perlayer:
        _lr_scales = [getattr(args, 'ttt_mlp_lr_scale', 2.0),
                      getattr(args, 'ttt_attn_lr_scale', 0.5), 1.0]
        optimizer = torch.optim.AdamW([
            {'params': mlp_params, 'lr': args.ttt_lr * _lr_scales[0]},
            {'params': attn_params, 'lr': args.ttt_lr * _lr_scales[1]},
            {'params': other_params, 'lr': args.ttt_lr * _lr_scales[2]},
        ], weight_decay=0.0, eps=1e-8)
    elif _use_adamw:
        _lr_scales = [1.0]
        optimizer = torch.optim.AdamW(ttt_params, lr=args.ttt_lr, weight_decay=0.0, eps=1e-8)
    else:
        _lr_scales = [1.0]
    ...
    for ci in range(num_chunks):
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                ...
                for _i, pg in enumerate(optimizer.param_groups):
                    pg['lr'] = args.ttt_lr * _lr_scales[min(_i, len(_lr_scales) - 1)] * _cos_scale
