...
class Hyperparameters:
    ...
    ttt_qk_lr_mult = float(os.environ.get("TTT_QK_LR_MULT", 0.5))
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer == "adamw":
        # Banks: mlp_up_bank/mlp_down_bank get higher LR, qo_bank/kv_bank get lower LR
        mlp_ps = [p for nm, p in ttt_named_params if nm in ("mlp_up_bank", "mlp_down_bank")]
        attn_ps = [p for nm, p in ttt_named_params if nm in ("qo_bank", "kv_bank")]
        excl_ids = {id(p) for p in mlp_ps} | {id(p) for p in attn_ps}
        other_ps = [p for p in ttt_params if id(p) not in excl_ids]
        optimizer = torch.optim.AdamW([
            {"params": mlp_ps, "lr": args.ttt_lr * args.ttt_mlp_lr_mult, "_base_lr_mult": args.ttt_mlp_lr_mult},
            {"params": attn_ps, "lr": args.ttt_lr * args.ttt_qk_lr_mult, "_base_lr_mult": args.ttt_qk_lr_mult},
            {"params": other_ps, "lr": args.ttt_lr, "_base_lr_mult": 1.0},
        ], betas=(0.9, 0.95), weight_decay=0.0)
        log0(f"ttt_sliding:optimizer=adamw mlp_lr={args.ttt_lr * args.ttt_mlp_lr_mult:.4f} "
             f"attn_lr={args.ttt_lr * args.ttt_qk_lr_mult:.4f} other_lr={args.ttt_lr:.4f}")
    else:
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                ...
                for pg in optimizer.param_groups:
                    pg['lr'] = args.ttt_lr * pg.get('_base_lr_mult', 1.0) * cos_scale
