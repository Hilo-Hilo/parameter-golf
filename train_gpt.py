...
def eval_val_sliding_ttt(
    ...
    # Freeze first N blocks; split remaining params by type for per-layer LR
    ...
    ttt_params = other_ttt_params + mlp_ttt_params

    log0(f"ttt_sliding:params unfrozen={sum(p.numel() for p in ttt_params)} "
         f"frozen={sum(p.numel() for p in base_model.parameters() if not p.requires_grad)}")

    mlp_lr = args.ttt_lr * 3.0
    other_lr = args.ttt_lr
    optimizer = torch.optim.AdamW(
        [{"params": other_ttt_params, "lr": other_lr, "initial_lr": other_lr},
         {"params": mlp_ttt_params, "lr": mlp_lr, "initial_lr": mlp_lr}],
