...
def ttt_eval(
    ...
    """Chunk-by-chunk TTT: score each chunk, then train on it."""
    seq_len = args.train_seq_len
    chunk_tokens = args.ttt_chunk_tokens
    total_val = val_tokens.numel() - 1

    # Freeze embeddings during TTT
    frozen_params = set()
    if args.ttt_freeze_embeds:
        for name, p in base_model.named_parameters():
            if "tok_emb" in name or "bigram" in name:
                p.requires_grad_(False)
                frozen_params.add(name)

    # Build per-layer TTT optimizer groups
    ttt_param_groups = []
    if args.ttt_perlayer:
        ...
        for name, p in base_model.named_parameters():
            if not p.requires_grad:
                continue
            if ".mlp.proj." in name:
                mlp_out_params.append(p)
            elif ".fc." in name or ".c_q." in name or ".c_k." in name or ".c_v." in name:
                input_params.append(p)
            else:
                other_params.append(p)
        ttt_param_groups = [
            {"params": mlp_out_params, "lr": args.ttt_lr * 3.0},
        ...
        ttt_param_groups = [{"params": trainable, "lr": args.ttt_lr}]

    ttt_opt = torch.optim.AdamW(ttt_param_groups, weight_decay=0.0)

    # EMA state for scoring
    ema_state = {k: v.detach().clone() for k, v in base_model.state_dict().items()}

    # Split val tokens into chunks
    chunk_starts = list(range(0, total_val, chunk_tokens))
    ...
    my_s = (num_chunks * rank) // world_size
    my_e = (num_chunks * (rank + 1)) // world_size
    ...
    for ci in range(num_chunks):
        cs = chunk_starts[ci]
        ce = min(cs + chunk_tokens, total_val)
        chunk_len = ce - cs

        # Only process chunks assigned to this rank for scoring,
        # but all ranks train on all chunks (for simplicity on single GPU)
        is_my_chunk = my_s <= ci < my_e

        # --- SCORE with EMA weights ---
        if is_my_chunk:
            # Swap to EMA weights for scoring
            raw_state = {k: v.detach().clone() for k, v in base_model.state_dict().items()}
            base_model.load_state_dict(ema_state, strict=True)
            base_model.eval()

            chunk_data = val_tokens[cs:ce + 1].to(device=device, dtype=torch.int64)
            # Process in seq_len-sized windows within the chunk
            num_seqs = chunk_len // seq_len
            if num_seqs > 0:
                chunk_x = chunk_data[:num_seqs * seq_len].reshape(num_seqs, seq_len)
                chunk_y = chunk_data[1:num_seqs * seq_len + 1].reshape(num_seqs, seq_len)

                with torch.inference_mode():
                    # Process in batches to avoid OOM
                    batch_seqs = min(args.eval_batch_seqs, num_seqs)
                    for bi in range(0, num_seqs, batch_seqs):
                        bx = chunk_x[bi:bi + batch_seqs]
                        by = chunk_y[bi:bi + batch_seqs]
                        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        ...
        # --- TRAIN on this chunk ---
        base_model.train()
        num_train_seqs = chunk_len // seq_len
        if num_train_seqs > 0:
            train_x = val_tokens[cs:cs + num_train_seqs * seq_len].to(device=device, dtype=torch.int64).reshape(num_train_seqs, seq_len)
            train_y = val_tokens[cs + 1:cs + num_train_seqs * seq_len + 1].to(device=device, dtype=torch.int64).reshape(num_train_seqs, seq_len)

            for epoch in range(args.ttt_epochs):
                # Cosine LR schedule
                if args.ttt_cosine:
                    progress = training_step / max(num_chunks * args.ttt_epochs, 1)
                ...
                batch_seqs = min(8, num_train_seqs)
                accum_steps = max(1, num_train_seqs // batch_seqs)
                for ai in range(accum_steps):
                    si = ai * batch_seqs
                    ei = min(si + batch_seqs, num_train_seqs)
                    bx = train_x[si:ei]
                    by = train_y[si:ei]
                    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                        loss = base_model(bx, by)
            ...
            with torch.no_grad():
                for name, param in base_model.named_parameters():
                    if name in ema_state:
                        ema_state[name].mul_(args.ttt_ema_decay).add_(param.data, alpha=1.0 - args.ttt_ema_decay)
                # Also update non-parameter buffers
                for name in ema_state:
                    if name not in dict(base_model.named_parameters()):
                        ema_state[name] = base_model.state_dict()[name].detach().clone()

        if log_fn and rank == 0 and (ci + 1) % 100 == 0:
            elapsed = time.perf_counter() - t_ttt
            running_bpb = 0.0
            if token_count.item() > 0:
                rl = (loss_sum / token_count).item()
                running_bpb = rl / math.log(2.0) * (token_count.item() / byte_count.item())
            log_fn(f"  ttt [{ci+1}/{num_chunks}] running_bpb={running_bpb:.6f} elapsed={elapsed:.0f}s")
...
def main() -> None:
    ...
    # Final evaluation on int6-roundtripped weights
    torch.cuda.synchronize()
    t_qeval = time.perf_counter()
    if args.ttt_enabled:
        log0(f"final_eval_mode:ttt lr={args.ttt_lr} epochs={args.ttt_epochs} "
             f"chunk={args.ttt_chunk_tokens} cosine={args.ttt_cosine} perlayer={args.ttt_perlayer}")
        q_val_loss, q_val_bpb = ttt_eval(
