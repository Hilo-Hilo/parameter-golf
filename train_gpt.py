...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer == "adamw" and args.ttt_perlayer_lr:
        # Per-layer LR groups built by matching param id to names
        ttt_param_ids = {id(p) for p in ttt_params}
        mlp_ids, attn_ids = set(), set()
        for name, p in base_model.named_parameters():
            ...
            if "mlp_up_bank" in name or "mlp_down_bank" in name:
                ...
                attn_ids.add(id(p))
        param_groups = [
            {"params": [p for p in ttt_params if id(p) in mlp_ids], "lr": args.ttt_lr * 2.0, "initial_lr": args.ttt_lr * 2.0},
            {"params": [p for p in ttt_params if id(p) in attn_ids], "lr": args.ttt_lr * 0.5, "initial_lr": args.ttt_lr * 0.5},
            {"params": [p for p in ttt_params if id(p) not in mlp_ids and id(p) not in attn_ids], "lr": args.ttt_lr, "initial_lr": args.ttt_lr},
        ]
        param_groups = [g for g in param_groups if g["params"]]
        optimizer = torch.optim.AdamW(param_groups, betas=(0.9, 0.95), weight_decay=0.0)
        log0(f"ttt_sliding:using AdamW per-layer-lr mlp_params={len(mlp_ids)} attn_params={len(attn_ids)}")
    elif args.ttt_optimizer == "adamw":
        optimizer = torch.optim.AdamW(ttt_params, lr=args.ttt_lr, betas=(0.9, 0.95), weight_decay=0.0)
        log0("ttt_sliding:using AdamW")
    else:
