...
class Hyperparameters:
    ...
    prune_before_quant_pct = float(os.environ.get("PRUNE_BEFORE_QUANT_PCT", "0.0"))
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_use_adamw:
        optimizer = torch.optim.AdamW(ttt_params, lr=args.ttt_lr, betas=(0.9, 0.999), weight_decay=args.ttt_adamw_wd)
    else:
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def quantize_int6_per_row(t: Tensor, clip_range: int = 31, prune_pct: float = 0.0) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if prune_pct > 0.0 and t32.ndim == 2:
        # Prune-then-quantize (arXiv:2603.18426): zero out the smallest-magnitude weights
        # per row before quantizing. Zeros compress better with lzma and reduce quant error
        # for remaining weights.
        threshold = torch.quantile(t32.abs(), prune_pct, dim=1, keepdim=True)
        t32 = t32 * (t32.abs() > threshold).float()
    if t32.ndim == 2:
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], prune_pct: float = 0.0):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t, prune_pct=prune_pct)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def main() -> None:
    ...
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, prune_pct=args.prune_before_quant_pct)
