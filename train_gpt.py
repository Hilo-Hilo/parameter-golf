...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer == "adamw":
        if args.ttt_perlayer_lr:
            param_set = {id(p) for p in ttt_params}
            ...
            for name, p in base_model.named_parameters():
                ...
                if "mlp" in name or "mlp_up_bank" in name or "mlp_down_bank" in name:
                    ...
                    other_ps.append(p)
            ttt_param_groups = [
                {"params": mlp_ps,   "lr": args.ttt_lr * 2.0, "base_lr": args.ttt_lr * 2.0},
                {"params": attn_ps,  "lr": args.ttt_lr * 0.5, "base_lr": args.ttt_lr * 0.5},
        ...
        optimizer = torch.optim.AdamW(
            ttt_param_groups, lr=args.ttt_lr, betas=(0.9, 0.95), eps=1e-8, weight_decay=0.0
        )
    else:
