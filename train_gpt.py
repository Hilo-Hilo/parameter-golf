...
class Hyperparameters:
    ...
    kv_gate_enabled = bool(int(os.environ.get("KV_GATE_ENABLED", "0")))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # KV-gate: use normalized K to gate V (zero extra parameters)
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.kv_gate:
            # K-gated V: use normalized K to modulate V (zero extra parameters)
            # 1 + tanh(k) has mean=1 at init (tanh(0)=0), range (0,2), stable gradient
            v = v * (1.0 + torch.tanh(k))
