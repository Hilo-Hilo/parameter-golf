...
class Hyperparameters:
    ...
    ttt_chunk_reset = bool(int(os.environ.get("TTT_CHUNK_RESET", "0")))
...
def eval_val_sliding_ttt(
    ...
    def _make_ttt_optimizer():
        if args.ttt_optimizer == "adamw":
            return torch.optim.AdamW(ttt_params, lr=args.ttt_lr, betas=(0.9, 0.999), weight_decay=0.01)
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                if args.ttt_const_lr:
                    chunk_lr = args.ttt_lr
                else:
                    chunk_lr = args.ttt_lr * 0.5 * (1.0 + math.cos(math.pi * ci / max(num_chunks - 1, 1)))
                if args.ttt_chunk_reset:
                    optimizer = _make_ttt_optimizer()
