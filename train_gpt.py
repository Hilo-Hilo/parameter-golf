...
def eval_val_sliding_ttt(
    ...
    _ttt_lr_scales: dict[int, float] = {}
    for _n, _p in base_model.named_parameters():
        if 'mlp_down_bank' in _n:
            _ttt_lr_scales[id(_p)] = 3.0
        elif 'qo_bank' in _n:
            _ttt_lr_scales[id(_p)] = 0.75
        else:
            _ttt_lr_scales[id(_p)] = 1.0
    _ttt_param_groups = [
        {'params': [p], 'lr': args.ttt_lr * _ttt_lr_scales.get(id(p), 1.0),
         'base_lr': args.ttt_lr * _ttt_lr_scales.get(id(p), 1.0)}
        for p in ttt_params
    ]
    optimizer = torch.optim.AdamW(_ttt_param_groups, betas=(0.9, 0.999), weight_decay=0.0)
