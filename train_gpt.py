...
class Hyperparameters:
    ...
    gluv_silu = bool(int(os.environ.get("GLUV_SILU", "0")))
    ...
    for name, p in base_model.named_parameters():
        ...
        if freeze:
            ...
            if args.ttt_perlayer_lr:
                is_mlp = ".mlp." in name
                lr_scale = 2.0 if is_mlp else 0.5
                ttt_param_groups.append({"params": [p], "lr": args.ttt_lr * lr_scale, "initial_lr": args.ttt_lr * lr_scale})

    log0(f"ttt_sliding:params unfrozen={sum(p.numel() for p in ttt_params)} "
         f"frozen={sum(p.numel() for p in base_model.parameters() if not p.requires_grad)}")

    if args.ttt_optimizer == "adamw":
        opt_groups = ttt_param_groups if args.ttt_perlayer_lr else ttt_params
        optimizer = torch.optim.AdamW(opt_groups, lr=args.ttt_lr, betas=(0.9, 0.95), weight_decay=0.0)
    else:
