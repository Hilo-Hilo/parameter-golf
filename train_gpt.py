...
def eval_val_sliding_ttt(
    ...
    # Freeze first N blocks; separate MLP banks for per-layer LR
    ...
    for name, p in base_model.named_parameters():
        ...
        if freeze:
            ...
            if "mlp_up_bank" in name or "mlp_down_bank" in name:
                mlp_ttt_params.append(p)
            else:
                other_ttt_params.append(p)

    ttt_params = mlp_ttt_params + other_ttt_params
    log0(f"ttt_sliding:params unfrozen={sum(p.numel() for p in ttt_params)} "
         f"frozen={sum(p.numel() for p in base_model.parameters() if not p.requires_grad)}")

    _mlp_ttt_lr = args.ttt_lr * 3.0
    _other_ttt_lr = args.ttt_lr
    optimizer = torch.optim.AdamW(
        [
            {"params": mlp_ttt_params, "lr": _mlp_ttt_lr, "initial_lr": _mlp_ttt_lr},
            {"params": other_ttt_params, "lr": _other_ttt_lr, "initial_lr": _other_ttt_lr},
        ],
