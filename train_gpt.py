...
def eval_val_sliding_ttt(
    ...
    if args.ttt_perlayer_lr:
        ...
        param_groups = [
            {"params": mlp_params, "lr": args.ttt_lr * args.ttt_mlp_lr_scale},
            {"params": attn_params, "lr": args.ttt_lr * args.ttt_attn_lr_scale},
    ...
    if args.ttt_optimizer == "adamw":
        optimizer = torch.optim.AdamW(param_groups, lr=args.ttt_lr, betas=(0.9, 0.95), eps=1e-8, weight_decay=0.0)
    else:
        # Default SGD; per-layer groups share the same momentum buffer logic
        flat_params = mlp_params + attn_params + other_params if args.ttt_perlayer_lr else ttt_params
        optimizer = torch.optim.SGD(flat_params, lr=args.ttt_lr, momentum=args.ttt_momentum)
    ...
    for ci in range(num_chunks):
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                ...
                if args.ttt_perlayer_lr:
                    lr_scales = [args.ttt_mlp_lr_scale, args.ttt_attn_lr_scale, 1.0]
                    for pg, lrs in zip(optimizer.param_groups, lr_scales):
                        pg['lr'] = args.ttt_lr * lrs * cos_scale
                else:
