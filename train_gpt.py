...
class Hyperparameters:
    ...
    ttt_perlayer = bool(int(os.environ.get("TTT_PERLAYER", "0")))  # per-layer LR groups for AdamW TTT
...
class TrigramHashEmbedding(nn.Module):
    """Order-3 n-gram hash embedding: uses prev-prev, prev, and current tokens."""
    ...
    def trigram_hash(self, tokens: Tensor) -> Tensor:
        ...
        out[..., 2:] = (
            torch.bitwise_xor(
                torch.bitwise_xor(51347 * t[..., 2:], 36313 * t[..., 1:-1]),
                27191 * t[..., :-2]
            ) % mod
        )
        return out.long()
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer == "adamw":
        if args.ttt_perlayer:
            ...
            param_groups = []
            if mlp_params:
                param_groups.append({"params": mlp_params, "lr": args.ttt_lr * 2.0})
            if attn_params:
                param_groups.append({"params": attn_params, "lr": args.ttt_lr * 0.5})
            if other_params:
