    ...
    if args.ttt_optimizer == "adamw":
        if args.ttt_perlayer_lr:
            ...
            mlp_params = [p for n, p in base_model.named_parameters()
                          if id(p) in ttt_param_set and ('mlp' in n)]
            attn_params = [p for n, p in base_model.named_parameters()
                           if id(p) in ttt_param_set and
                           any(k in n for k in ('qo_bank', 'kv_bank', 'attn', 'q_gain', 'vr_lambda'))
                           and id(p) not in set(id(x) for x in mlp_params)]
            other_params = [p for p in ttt_params
                            if id(p) not in set(id(x) for x in mlp_params)
                            and id(p) not in set(id(x) for x in attn_params)]
            ttt_pgs = [g for g in [
            ...
            optimizer = torch.optim.AdamW(ttt_pgs, lr=args.ttt_lr, betas=(0.9, 0.95), weight_decay=0.0)
        else:
...
def main() -> None:
    ...
    eval_model = GPT(
        ...
        v_norm=args.v_norm, silu_v=args.silu_v,
    ).to(device).bfloat16()
