...
def eval_val_sliding_ttt(
    ...
    for name, p in base_model.named_parameters():
        ...
        if freeze:
            ...
            ttt_named.append((name, p))
    ttt_params = [p for _, p in ttt_named]
    ...
    if args.ttt_optimizer == "adamw":
        ...
        for n, p in ttt_named:
            if "mlp_up_bank" in n or "mlp_down_bank" in n or ".mlp." in n:
                mlp_ps.append(p)
            elif "qo_bank" in n or "kv_bank" in n or ".attn." in n:
                attn_ps.append(p)
            else:
                other_ps.append(p)
        pg_groups = []
        if mlp_ps:
            pg_groups.append({"params": mlp_ps, "lr": args.ttt_lr * args.ttt_mlp_lr_mult, "base_lr": args.ttt_lr * args.ttt_mlp_lr_mult})
        if attn_ps:
            pg_groups.append({"params": attn_ps, "lr": args.ttt_lr * args.ttt_attn_lr_mult, "base_lr": args.ttt_lr * args.ttt_attn_lr_mult})
        if other_ps:
            pg_groups.append({"params": other_ps, "lr": args.ttt_lr, "base_lr": args.ttt_lr})
        optimizer = torch.optim.AdamW(pg_groups, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.0)
        log0(f"ttt_sliding:optimizer=adamw mlp_params={len(mlp_ps)} attn_params={len(attn_ps)} other_params={len(other_ps)} "
