...
class Hyperparameters:
    ...
    ttt_bigram_lr_mult = float(os.environ.get("TTT_BIGRAM_LR_MULT", "1.0"))
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_perlayer:
        bigram_p, mlp_p, attn_p, other_p = [], [], [], []
        for name, p in base_model.named_parameters():
            if not p.requires_grad:
                continue
            if "bigram" in name or ".smear." in name:
                bigram_p.append(p)
            elif "mlp_up_bank" in name or "mlp_down_bank" in name or "mlp." in name:
                mlp_p.append(p)
        ...
        param_groups = []
        if bigram_p:
            param_groups.append({'params': bigram_p, 'lr': lr_base * args.ttt_bigram_lr_mult, '_lr_mult': args.ttt_bigram_lr_mult})
        if mlp_p:
            param_groups.append({'params': mlp_p, 'lr': lr_base * args.ttt_mlp_lr_mult, '_lr_mult': args.ttt_mlp_lr_mult})
        if attn_p:
            param_groups.append({'params': attn_p, 'lr': lr_base * args.ttt_attn_lr_mult, '_lr_mult': args.ttt_attn_lr_mult})
        if other_p:
            param_groups.append({'params': other_p, 'lr': lr_base, '_lr_mult': 1.0})
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                ...
                for pg in optimizer.param_groups:
                    pg['lr'] = cos_lr_base * pg.get('_lr_mult', 1.0)
