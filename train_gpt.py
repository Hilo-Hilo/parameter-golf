...
def eval_val_sliding_ttt(
    ...
    if args.ttt_perlayer_lr:
        mlp_params = [p for n, p in zip(ttt_param_names, ttt_params) if "mlp_up_bank" in n or "mlp_down_bank" in n or ".mlp." in n]
        attn_params = [p for n, p in zip(ttt_param_names, ttt_params) if "qo_bank" in n or "kv_bank" in n or ".attn." in n]
        other_params = [p for n, p in zip(ttt_param_names, ttt_params) if "mlp_up_bank" not in n and "mlp_down_bank" not in n and ".mlp." not in n and "qo_bank" not in n and "kv_bank" not in n and ".attn." not in n]
        param_groups = [
            {"params": mlp_params, "lr": args.ttt_lr * args.ttt_mlp_lr_scale, "base_lr": args.ttt_lr * args.ttt_mlp_lr_scale},
            {"params": attn_params, "lr": args.ttt_lr * args.ttt_attn_lr_scale, "base_lr": args.ttt_lr * args.ttt_attn_lr_scale},
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        relu_alpha=args.mlp_relu_alpha,
    ).to(device).bfloat16()
