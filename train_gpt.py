...
class Block(nn.Module):
    def __init__(
        ...
        if ddl_enabled:
            ...
            self.ddl_beta = nn.Parameter(torch.tensor(0.0, dtype=torch.float32))
        else:
            self.ddl_v = None
            self.ddl_beta = None
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.ddl_v is not None:
            v_norm = F.normalize(self.ddl_v.to(dtype=x_in.dtype), dim=0)
            beta = torch.tanh(self.ddl_beta.to(dtype=x_in.dtype))
            coeff = (x_in * v_norm).sum(-1, keepdim=True)
            x_in = x_in + beta * coeff * v_norm
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_use_adamw:
        optimizer = torch.optim.AdamW(
            ...
            betas=(0.9, 0.999), eps=1e-8, weight_decay=args.ttt_adamw_wd,
        )
    else:
