...
- Keep novel Parameter Golf experiments on dedicated `approach/<name>` branches; treat `main` as the shared integration baseline. Always commit and push structural or controller script changes directly to the `main` branch.
...
- For autonomous commits, use `git add .` to ensure genuinely new files are captured, rather than `git add -u`, and avoid suppressing commit errors with `|| true`.
