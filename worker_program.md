...
4. **No Privilege Escalation:** Do not use SSH, check live RunPod state, or decide which pod to use. The shell controller manages the pod pool and queueing. Pods are dumb executors.
...
2. **Execute:** If novel, the controller creates the branch, commits your edits, pushes to GitHub, and dispatches the exact commit to a RunPod.
